...
import { Bell, Plus, RefreshCw, Trash2, TrendingUp, TrendingDown, Clock3, DollarSign, Eye, AlertTriangle, CheckCircle2, Settings2, Database, Pencil } from "lucide-react";
...
function loadState() {
  ...
  try {
    ...
    if (!raw) return { watchlist: DEFAULT_WATCHLIST, settings: DEFAULT_SETTINGS };
...
function saveState(watchlist, settings) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ watchlist, settings }));
}
...
function calcSignal(item, quote, settings) {
  if (!quote) {
    return {
      label: "資料不足",
...
function randomizeQuote(quote) {
  ...
  return {
    ...
    changePct: Number((quote.changePct + (Math.random() - 0.5) * 0.5).toFixed(2)),
...
async function fetchQuotes(symbols, useDemoData) {
  if (useDemoData) {
    const result = {};
    symbols.forEach((symbol) => {
      if (DEMO_QUOTES[symbol]) result[symbol] = randomizeQuote(DEMO_QUOTES[symbol]);
    });
    return result;
  }

  // 正式上線時，改接你自己的 API。
  // 建議格式：/api/quotes?symbols=ITA,UFO,SCHD,DTCR
  // 回傳：{ ITA: { price, changePct, volume, bid, ask, previousClose }, ... }
  const response = await fetch(`/api/quotes?symbols=${encodeURIComponent(symbols.join(","))}`);
  if (!response.ok) throw new Error("報價 API 讀取失敗");
...
export default function ETFOrderDashboard() {
  ...
  async function refreshQuotes() {
    setLoading(true);
    try {
      const symbols = watchlist.map((item) => item.symbol.trim().toUpperCase()).filter(Boolean);
  ...
  const enriched = useMemo(() => {
    return watchlist.map((item) => {
      ...
      const gapPct = quote ? ((quote.price - item.targetBuy) / item.targetBuy) * 100 : null;
      return {
        ...item,
        quote,
        amount,
  ...
  const now = new Date();
  const marketWindow = isInsideWindow(now, settings.marketWindowStart, settings.marketWindowEnd);

  function openCreateDialog() {
    setEditingSymbol(null);
    setForm({ symbol: "", name: "", category: "", targetBuy: "", shares: "", note: "" });
    setDialogOpen(true);
  }
  ...
  function submitItem(e) {
    ...
    if (!payload.symbol || !payload.name || !payload.category || !payload.targetBuy || !payload.shares) return;

    setWatchlist((prev) => {
      const exists = prev.some((item) => item.symbol === payload.symbol);
      if (editingSymbol || exists) {
        return prev.map((item) => (item.symbol === (editingSymbol || payload.symbol) ? payload : item));
      }
      return [...prev, payload];
    });
    setDialogOpen(false);
  ...
  const sorted = [...enriched].sort((a, b) => b.signal.score - a.signal.score);
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-6">
        ...
        >
          <div className="space-y-3">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold tracking-tight">正式可上線的每日觀察面板</h1>
              <p className="mt-3 max-w-3xl text-slate-600 leading-7">
                這個版本已具備你要的核心結構：可新增觀察標的、儲存自訂買點與股數、每日刷新報價、計算預估成交金額，並依你的規則判斷是否適合下單。
                目前預設使用示意資料；把報價 API 接上後，就能直接部署成你每天隨時打開就用的實站。
              </p>
            </div>
          </div>

          <div className="flex flex-wrap gap-3">
            <Button variant="outline" className="rounded-2xl" onClick={refreshQuotes}>
              ...
              <DialogContent className="sm:max-w-2xl rounded-3xl">
                ...
                <form onSubmit={submitItem} className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div className="space-y-2">
                    <Label>代號</Label>
                    <Input value={form.symbol} onChange={(e) => setForm({ ...form, symbol: e.target.value })} placeholder="例如 SOXX" />
                  </div>
                  <div className="space-y-2">
                    <Label>名稱</Label>
                    <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="例如 iShares Semiconductor ETF" />
                  </div>
                  <div className="space-y-2">
                    <Label>主題類型</Label>
                    <Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="例如 半導體" />
                  </div>
                  <div className="space-y-2">
                    <Label>理想買點</Label>
                    <Input type="number" step="0.01" value={form.targetBuy} onChange={(e) => setForm({ ...form, targetBuy: e.target.value })} placeholder="例如 235.50" />
                  </div>
                  <div className="space-y-2">
                    <Label>預計股數</Label>
                    <Input type="number" step="1" value={form.shares} onChange={(e) => setForm({ ...form, shares: e.target.value })} placeholder="例如 3" />
                  </div>
                  <div className="space-y-2">
                    <Label>備註</Label>
                    <Input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="例如 長期慢慢增倉" />
                  </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
          <InfoCard title="觀察標的數" value={`${watchlist.length} 檔`} icon={Eye} sub="可自訂新增 / 編輯 / 刪除" />
          <InfoCard title="目前可考慮下單" value={`${summary.ready} 檔`} icon={CheckCircle2} sub="依你的買點與日內狀態判斷" />
          <InfoCard title="偏熱不宜追價" value={`${summary.hot} 檔`} icon={TrendingUp} sub="日內動能過熱或高於理想價" />
          <InfoCard title="預估總成交金額" value={`$${formatNumber(summary.totalAmount, 2)}`} icon={DollarSign} sub="依目前價格 × 預計股數" />
          <InfoCard
            title="最佳觀察時段"
        ...
        <Tabs defaultValue="watchlist" className="space-y-4">
          <TabsList className="rounded-2xl bg-white border border-slate-200">
            <TabsTrigger value="watchlist" className="rounded-2xl">觀察清單</TabsTrigger>
            <TabsTrigger value="signals" className="rounded-2xl">下單訊號</TabsTrigger>
            <TabsTrigger value="settings" className="rounded-2xl">設定</TabsTrigger>
          </TabsList>
                      ...
                      {watchlist.map((item) => {
                        ...
                        return (
                          <tr key={item.symbol} className="border-b border-slate-100">
                            ...
                            <td className="py-4 pr-4"><Badge variant="outline" className="rounded-full">{item.category}</Badge></td>
                            <td className="py-4 pr-4 font-medium">{quote ? `$${formatNumber(quote.price)}` : "—"}</td>
                            ...
                            <td className="py-4 pr-4">
                              {quote ? (
                                <span className={`inline-flex items-center gap-1 ${quote.changePct >= 0 ? "text-emerald-700" : "text-rose-700"}`}>
                                  {quote.changePct >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                                  {formatNumber(quote.changePct)}%
                                </span>
                              ) : "—"}
                            </td>
                            <td className="py-4 pr-4">{quote ? formatNumber(quote.volume, 0) : "—"}</td>
                            <td className="py-4 pr-4">{sPct !== null ? `${formatNumber(sPct)}%` : "—"}</td>
                            <td className="py-4 pr-4">
                              <div className="flex gap-2">
                                <Button variant="outline" size="icon" className="rounded-2xl" onClick={() => openEditDialog(item)}>
                                  <Pencil className="h-4 w-4" />
                                </Button>
                                <Button variant="outline" size="icon" className="rounded-2xl" onClick={() => deleteItem(item.symbol)}>
                    ...
                    <Card className="rounded-3xl border-slate-200 shadow-sm h-full">
                      <CardHeader>
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm ${item.signal.tone}`}>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <Metric label="價格偏離" value={item.gapPct !== null ? `${formatNumber(item.gapPct)}%` : "—"} />
                          <Metric label="日內漲跌" value={item.quote ? `${formatNumber(item.quote.changePct)}%` : "—"} />
          ...
          <TabsContent value="settings" className="space-y-4">
            <div className="grid lg:grid-cols-2 gap-4">
              <Card className="rounded-3xl border-slate-200 shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><Settings2 className="h-5 w-5" /> 規則設定</CardTitle>
                ...
                <CardContent className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input value={settings.marketWindowStart} onChange={(e) => setSettings({ ...settings, marketWindowStart: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                      <Label>主要觀察結束時間</Label>
                      <Input value={settings.marketWindowEnd} onChange={(e) => setSettings({ ...settings, marketWindowEnd: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                      <Label>過熱門檻（%）</Label>
                      <Input type="number" step="0.1" value={settings.hotMoveThresholdPct} onChange={(e) => setSettings({ ...settings, hotMoveThresholdPct: Number(e.target.value) })} />
                    </div>
                    <div className="space-y-2">
                      <Label>可接受偏離（%）</Label>
                      <Input type="number" step="0.1" value={settings.acceptableGapPct} onChange={(e) => setSettings({ ...settings, acceptableGapPct: Number(e.target.value) })} />
                    </div>
                    <div className="space-y-2">
                      <Label>最大價差（%）</Label>
                      <Input type="number" step="0.01" value={settings.maxSpreadPct} onChange={(e) => setSettings({ ...settings, maxSpreadPct: Number(e.target.value) })} />
                    </div>
                    <div className="space-y-2">
                      <Label>自動刷新秒數</Label>
                      <Input type="number" step="5" value={settings.autoRefreshSeconds} onChange={(e) => setSettings({ ...settings, autoRefreshSeconds: Number(e.target.value) })} />
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="rounded-3xl border-slate-200 shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><Bell className="h-5 w-5" /> 資料來源與上線說明</CardTitle>
                  <CardDescription>這一版前端已完成主要互動，正式上線前還差最後一段報價 API。</CardDescription>
                </CardHeader>
                <CardContent className="space-y-5 text-sm text-slate-600 leading-6">
                  <div className="flex items-center justify-between rounded-2xl border border-slate-200 p-4">
                    ...
                    <Switch checked={settings.useDemoData} onCheckedChange={(checked) => setSettings({ ...settings, useDemoData: checked })} />
                  </div>
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4">
                    ...
                    <div>Next.js + Vercel + 一個穩定的報價 API。這樣最符合你之前網站的延伸方向，也最容易正式上線。</div>
                  </div>
                </CardContent>
              </Card>
            </div>
...
function InfoCard({ title, value, sub, icon: Icon }) {
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
